...
export function MarketplacePage() {
  return <ModuleShell active="/marketplace" eyebrow="NEXUS MARKET • EM PREPARAÇÃO" title="Marketplace ainda sem anúncios reais." description="Produtos e preços só serão exibidos depois que vendedores reais forem cadastrados e verificados." action={<div className="usage-pill"><span>▣</span><p><strong>Sem ofertas ativas</strong><small>Nenhum preço ou avaliação fictícia</small></p></div>}><div className="market-banner"><div><span>VENDA NO NEXUS</span><strong>Área de vendedores em desenvolvimento.</strong><p>O cadastro será aberto após a ativação da autenticação e dos pagamentos.</p></div><button disabled aria-describedby="seller-status">Cadastro ainda fechado</button><small id="seller-status" className="sr-only">A autenticação e os pagamentos do marketplace ainda não foram ativados.</small></div><div className="product-grid">{marketplaceAreas.map((area) => <article key={area[0]}><div className={`product-art ${area[2]}`}><span>▣</span><em>EM PREPARAÇÃO</em></div><span>Categoria planejada</span><h2>{area[0]}</h2><p><small>{area[1]}</small></p><div><strong>Sem ofertas</strong><button disabled>Ainda não disponível</button></div></article>)}</div></ModuleShell>;
}

const learningTracks = [
  { title: "Desenvolvimento de jogos", description: "Doze missões do protótipo ao produto", color: "pink", href: "/aprender/game-dev", cover: "12 MISSÕES" },
  { title: "Negócios digitais", description: "Conteúdo prático em preparação", color: "violet" },
  { title: "Inteligência artificial", description: "Conteúdo prático em preparação", color: "cyan" },
  { title: "Criação de sites", description: "Conteúdo prático em preparação", color: "blue" },
];

export function LearnPage() {
  return <ModuleShell active="/aprender" eyebrow="NEXUS ACADEMY • PRIMEIRA TRILHA" title="Aprenda hoje. Entregue amanhã." description="A primeira trilha prática está aberta. As demais áreas continuam visíveis somente como planejamento, sem cursos ou certificados fictícios."><div className="learning-overview"><div><span>▤</span><p><strong>1 trilha publicada</strong><small>Wayne Game Lab disponível</small></p></div><div><span>◷</span><p><strong>Progresso local</strong><small>Salvo somente neste dispositivo</small></p></div><div><span>◆</span><p><strong>Entrega antes do certificado</strong><small>Sem credencial fictícia</small></p></div></div><div className="course-grid">{learningTracks.map((track) => <article key={track.title}><div className={`course-cover ${track.color}`}><span>{track.href ? "♢" : "▤"}</span><em>{track.cover || "EM PREPARAÇÃO"}</em></div><span>{track.href ? "TRILHA DISPONÍVEL" : "TRILHA PLANEJADA"}</span><h2>{track.title}</h2><p>{track.description}</p>{track.href ? <a href={track.href}>Começar agora →</a> : <button disabled>Ainda não disponível</button>}</article>)}</div></ModuleShell>;
}
